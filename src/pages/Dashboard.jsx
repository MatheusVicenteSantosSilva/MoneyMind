...
import { 
    ...
    ArrowUpRight,
    ArrowDownRight,
    Trash2 // NOVO: Ícone de lixeira reintroduzido
} from 'lucide-react';
...
// NOVO: Importe os componentes do AlertDialog (REINTRODUZIDO)
// LEMBRE-SE: ESTES CAMINHOS PRECISAM SER CASE-SENSITIVE NO VERCEL
import { 
    AlertDialog, 
    AlertDialogAction, 
    AlertDialogCancel, 
...
const Dashboard = () => {
    // Reintroduzindo deleteTransaction
    ...
    // Função para lidar com a exclusão da transação (Tornada assíncrona novamente por boa prática)
    const handleDeleteTransaction = async (id, description) => {
        try {
            // Usa await, se seu hook useTransactions for assíncrono (melhor prática)
            ...
            if (result && result.success) {
                toast({
                 ...
                 // Trata caso a exclusão falhe no hook (ex: ID não encontrado)
                 throw new Error("Falha na exclusão no hook.");
            }
        } catch (error) {
            toast({
                title: "Erro ao excluir",
                description: `Não foi possível remover a transação. Detalhe: ${error.message || 'Erro desconhecido'}`,
                variant: "destructive",
            });
        }
    };
        ...
        <>
            ...
            <div className="min-h-screen p-4 md:p-6">
                <div className="max-w-7xl mx-auto space-y-6">
                    ...
                    {/* Recent Transactions (BLOCO ATUALIZADO COM BOTÃO DE EXCLUSÃO) */}
                                        ...
                                        {recentTransactions.map((transaction, index) => (
                                            ...
                                            >
                                                ...
                                                <div className="flex items-center space-x-4">
                                                    ...
                                                    {/* Botão de Exclusão com Modal de Confirmação (REINTRODUZIDO) */}
                                                    <AlertDialog>
